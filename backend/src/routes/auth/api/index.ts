...
export const logout = async (
  ...
  return _logout(req, res, next).catch(
    (err) => new ApiError("Error while signing out", err, 500),
  );
};
    ...
    if (err) {
      LOGGER.warn(
        `Error while signing out user ${
          (req.user as IDtoUser).sub ?? "unknown"
        }: <${err}>`,
      );
...
export const authCheck = async (
  ...
  return getUserInfo(req.user as IDtoUser)
    ...
    .catch((err) => {
      next(new ApiError("Unauthenticated request", err, 401));
    });
};
