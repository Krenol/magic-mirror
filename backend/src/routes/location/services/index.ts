...
export const buildGeocodeUrl = async (req: Request) => {
  ...
  const queryParams = queryList.map((val) => `${val.name}=${val.value}`).join('&');
  return `${GEOCODE_URL}/search?${queryParams}`;
};

export const handleGeocodeResponse = async (res: Response, response: TResponse) => {
  if (response.body.length === 0) {
    throw new ApiError(response.body.reason ?? 'Geolocation could not be found', new Error(), 404);
  }
