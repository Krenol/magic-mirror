...
export class RegexParameterValidator extends ParameterValidator {
    ...
    constructor(paramName: string, validationRegex: RegExp, type: EParamType, required: boolean = false) {
    ...
    protected validateParameter(req: Request, res: Response, next: NextFunction) {
        ...
        LOGGER.error(`${this._type} parameter ${this._paramName} not matching regex ${this._validationRegex}- raising error`);
        return next(new ApiError(`Invalid ${this._type} parameter ${this._paramName}!`, undefined, 400));
    }
}