...
export class CustomParameterValidator extends ParameterValidator {
  ...
  constructor(paramName: string, validationFunc: IValidationFunc, type: EParamType, required = false) {
  ...
  protected validateParameter(req: Request, res: Response, next: NextFunction) {
    ...
    if (param) {
      return this._validationFunc(param).then((valid) => this.handleValidationResponse(valid, next));
    }
  ...
  private handleValidationResponse = async (valid: boolean, next: NextFunction) => {
    ...
    LOGGER.error(`${this._type} parameter ${this._paramName} not matching validation function - raising error`);
    return next(new ApiError(`${this._type} parameter ${this._paramName} is invalid!`, undefined, 400));
  };
}
