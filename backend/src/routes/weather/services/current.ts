...
export const handleCurrentWeatherResponse = async (
  res: Response,
  response: TResponse,
): Promise<Response> => {
...
const createResponseJson = async (response: any): Promise<CurrentWeather> => {
  const isDay = await sunIsCurrentlyUp(
  ...
  return {
    ...
    weather_icon: await getWeatherIconFromWeathercode(
      isDay,
      response.current_weather.weathercode,
    ),
    description: await getWeatherDescription(
      response.current_weather.weathercode,
    ),
  };
};
