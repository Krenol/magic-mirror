...
mkcert -key-file ./backend/ssl/express.key -cert-file ./backend/ssl/express.pem $hostname backend $addtional_hostnames
...
echo "$(cat $(mkcert -CAROOT)/rootCA.pem)" > ./backend/rootCa.pem
