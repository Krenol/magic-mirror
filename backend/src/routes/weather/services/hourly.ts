...
export const handleHourlyWeatherResponse = async (
  ...
  if (response.status === 200) {
    ...
    throw new ApiError(
      response.body.reason ?? "Error while calling weather API",
      new Error(),
      400,
    );
  } else {
    throw new ApiError(
      "Error while retrieving the current weather",
      new Error(),
      400,
    );
  }
...
export const getForecastHours = async (req: Request): Promise<number> => {
  const hour_query_param = parseInt(
    ((req.query.hours as string) ?? MAX_HOURLY_FORECAST_HOURS).toString(),
  );
...
const createResponse = async (
  ...
  return res
    .status(200)
    .json(await createResponseJson(response, forecast_hours));
};
...
const createForecastArray = async (
  ...
  for (let i = 0; i < count; i++) {
    if (
      await isValidHourlyForecastTime(response.hourly.time[i], forecast_hours)
    ) {
...
const isValidHourlyForecastTime = async (
  time: string,
  forecast_hours: number,
): Promise<boolean> => {
  return (
    (await timeHasPassed(time)) === false &&
    (await exceedsMaxForecastTime(time, forecast_hours)) === false
  );
};
...
const createForecastHour = async (
  ...
  const sunIsUp = await timeIsDuringDay(
    time,
