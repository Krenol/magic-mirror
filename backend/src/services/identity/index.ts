...
export const serializeUser = (user: Express.User, done: (err: any, id?: unknown) => void) => {
    LOGGER.info(`Serialize user with id ${(user as IDtoUser).sub}`);
    done(null, (user as IDtoUser).sub)
}

export const deserializeUser = (id: string, done: (err: any, user?: false | Express.User | null | undefined) => void) => {
