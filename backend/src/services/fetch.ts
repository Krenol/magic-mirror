...
export const fetchJsonGoogleAuthRefresh = async (
  url: string,
  user: IDtoUser,
  options: any = {},
  logUrl: string | undefined = undefined,
...
const _fetchJsonGoogleAuthRefresh = async (
  ...
  return getAuthenticationHeader(user)
    ...
    .then((rsp) =>
      handleGoogleAuthRefreshResponse(
        rsp,
        url,
        user,
        options,
...
const handleGoogleAuthRefreshResponse = async (
  response: TResponse,
  url: string,
  user: IDtoUser,
  options: any = {},
  ...
  if (response.status === 401 && refreshRetries > 0) {
    LOGGER.info(
      `Unauthenticated Google Request. Retry with refreshed token for user ${user.sub}`,
    );
...
const handleTokenRefresh = async (
  ...
  return userTokenRefresh(user).then((u) =>
    _fetchJsonGoogleAuthRefresh(url, u, options, --refreshRetries, logUrl),
  );
};
