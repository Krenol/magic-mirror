...
export abstract class ParameterValidator {
  ...
  protected readonly _paramContainsMap = new Map<
    EParamType,
    (req: Request, paramName: string) => Promise<boolean>
  >([
    [EParamType.query, requestQueryContainsParam],
    [EParamType.request, requestContainsParam],
  ]);
  protected readonly _paramRetrieveMap = new Map<
    EParamType,
    (req: Request) => string
  >([
    [
      EParamType.query,
      (req: Request) =>
        ((req.query[this._paramName] as string) ?? "").toString(),
    ],
    [
      EParamType.request,
      (req: Request) => (req.params[this._paramName] ?? "").toString(),
    ],
  ]);
  ...
  public validate = async (
    ...
    if (func) {
      ...
      if (!exists && !this._required) {
        LOGGER.info(
          `Non required ${this._type} parameter ${this._paramName} not existing in request - continuing`,
        );
        return next();
      } else if (!exists && this._required) {
        LOGGER.error(
          `Required ${this._type} parameter ${this._paramName} not existing in request - raising error`,
        );
        return next(
          new ApiError(
            `${this._type} parameter ${this._paramName} is required!`,
            undefined,
            400,
          ),
        );
  ...
  protected abstract validateParameter(
    req: Request,
    res: Response,
    next: NextFunction,
  ): void;
