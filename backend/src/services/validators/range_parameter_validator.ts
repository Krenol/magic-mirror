...
export class RangeParameterValidator extends ParameterValidator {
  ...
  protected validateParameter(req: Request, res: Response, next: NextFunction) {
    ...
    if (paramStr) {
      ...
      LOGGER.error(
        `${this._type} parameter ${this._paramName} is out of range [${this._range.min},${this._range.max}] - raising error`,
      );
      return next(
        new ApiError(
          `${this._type} parameter ${this._paramName} out of range [${this._range.min},${this._range.max}]!`,
          undefined,
          400,
        ),
      );
