...
router.get('/auth/login', passport.authenticate(LOGIN_STRATEGY_NAME, <object>{
    scope: SCOPES,
    accessType: 'offline',
    prompt: 'consent',
}));

router.get(GOOGLE_LOGIN_CALLBACK_CONTEXT, passport.authenticate(LOGIN_STRATEGY_NAME,
    {
...
router.get('/auth/register', passport.authenticate(REGISTER_STRATEGY_NAME, <object>{
    scope: SCOPES,
    accessType: 'offline',
    prompt: 'consent',
}));
