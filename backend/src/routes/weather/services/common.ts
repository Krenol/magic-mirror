...
export const getWeatherIconFromWeathercode = async (
  useDayIcons: boolean,
  weathercode: number,
): Promise<string> => {
...
export const getWeatherDescription = async (
  weathercode: number,
): Promise<string> => {
  const WeatherCodeEntry = await getWeatherCodeEntry(weathercode);
...
const getWeatherCodeEntry = async (
  weathercode: number,
): Promise<WeatherCodeEntry> => {
  const WeatherCodeEntry = weather_code_list.find(
    (item) => item.weathercode == weathercode,
  );
...
export const sunIsCurrentlyUp = async (
  sunrise: string,
  sunset: string,
): Promise<boolean> => {
  const now = new Date();
...
export const dateIsDuringDay = async (
  time: Date,
  sunrise: Date,
  sunset: Date,
): Promise<boolean> => {
...
export const timeIsDuringDay = async (
  time: string,
  sunrise: string,
  sunset: string,
): Promise<boolean> => {
