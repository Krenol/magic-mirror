    ...
    describe(`Unit testing the ${ROUTE}/forecast route`, () => {
        it(`should return OK status for ok test params`, async () => {
            const response = await request(app)
                .get(`${ROUTE}/forecast?latitude=${location_params.ok_latitude}&longitude=${location_params.ok_longitude}&days=${forecast_params.okay_days}`);
            assert.equal(response.status, 200);
        ...
        it(`should return 400 status for bad test params`, async () => {
            return request(app)
                .get(`${ROUTE}/forecast?latitude=${location_params.fault_latitude}&longitude=${location_params.fault_longitude}`)
                .then(response => assert.equal(response.status, 400))
        });
        it(`should return 400 status for 1000 day forcast`, async () => {
            return request(app)
                .get(`${ROUTE}/forecast?latitude=${location_params.ok_latitude}&longitude=${location_params.ok_longitude}&days=${forecast_params.too_many_days}`)
                .then(response => assert.equal(response.status, 400))
        });
        it(`should return 400 status for negative day forcast`, async () => {
            return request(app)
                .get(`${ROUTE}/forecast?latitude=${location_params.ok_latitude}&longitude=${location_params.ok_longitude}&days=${forecast_params.neg_days}`)
                .then(response => assert.equal(response.status, 400))
        });
    });
})
