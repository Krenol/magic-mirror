...
export abstract class ParameterValidator {
    ...
    constructor(paramName: string, type: EParamType, required: boolean = false) {
        this._paramName = paramName;
        this._required = required;
        this._type = type;
    }

    protected getParameter = (req: Request): string => this._paramRetrieveMap.get(this._type)!(req);
    ...
    public validate = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        ...
        const exists = await this._paramContainsMap.get(this._type)!(req, this._paramName);
